...
// Lazy loading pages to ensure separate chunks in build
const Dashboard = lazy(() => import('./pages/Dashboard'));
const CustomerList = lazy(() => import('./pages/CustomerList'));
const CustomerForm = lazy(() => import('./pages/CustomerForm'));
const FacilityEstimation = lazy(() => import('./pages/FacilityEstimation'));
const Reports = lazy(() => import('./pages/Reports'));
const PrintProfile = lazy(() => import('./pages/PrintProfile'));
  ...
  return (
    <div className="flex h-screen w-full bg-[#f8f9ff] text-slate-900 overflow-hidden font-['Vazirmatn'] relative">
      ...
      <div className="flex w-full h-full overflow-hidden">
        {!isPrintPage && !isAuthPage && <Sidebar isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} />}
        
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
          {!isPrintPage && !isAuthPage && (
            <header className="shrink-0 flex items-center justify-between px-6 bg-white/90 backdrop-blur-md border-b border-slate-100 h-20 shadow-sm z-50">
              <div className="flex items-center gap-4">
                ...
                <div className="hidden md:flex flex-col text-right">
                  <span className="text-[14px] font-black text-slate-900 leading-tight">مدیر سامانه</span>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            <div className={`w-full h-full ${isPrintPage || isAuthPage ? 'p-0' : 'p-4 sm:p-6 lg:p-8'}`}>
